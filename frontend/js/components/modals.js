...
const Modals = {
   ...
   */
  openWhatsAppSender: function(phone, name, amount, customerId = "", preferredTemplateKey = "") {
    if (!this.container) this.init();
    ...
    if (preferredTemplateKey && templates[preferredTemplateKey]) {
      defaultKey = preferredTemplateKey;
    } else if (amount > 0 && Number(amount) > 100000) {
      defaultKey = "supplier_cashflow";
    }
  ...
  /**
   * Dedicated WhatsApp Templates Library Showcase Modal (Filtered / Unlocked according to plans)
   */
  openWhatsAppTemplatesLibrary: function(initialCustomerId = "") {
    if (!this.container) this.init();
    const isPro = Store.isPro();
    const biz = (Store.state.settings && Store.state.settings.BUSINESS_NAME) || "Apex Supplies";
    const customers = (Store.getWithStale && Store.getWithStale("customers").data) || Store.state.customers || [];
    
    // Pick active selected debtor for realistic preview
    let selectedCust = customers.find(c => c.CustomerID === initialCustomerId) || customers[0] || {
      CustomerID: "CUS-DEMO",
      CustomerName: "Sample Client",
      Phone: "9876543210",
      metrics: { totalOutstanding: 75000 }
    };

    const name = selectedCust.CustomerName || "Client";
    const amount = (selectedCust.metrics && selectedCust.metrics.totalOutstanding) || 75000;
    const phone = selectedCust.Phone || selectedCust.WhatsApp || "";
    const templates = this.getWhatsAppTemplates(name, amount, biz);
    this._waLibraryTemplates = templates;
    this._waLibraryCustomer = selectedCust;
    ...
    this.container.innerHTML = `
      <div class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-5">
        <div class="bg-white rounded-2xl shadow-2xl max-w-3xl w-full overflow-hidden flex flex-col max-h-[92vh] animate-in fade-in zoom-in-95 duration-200">
          <!-- Header -->
          <div class="px-6 py-4 bg-gradient-to-r ${isPro ? 'from-emerald-600 via-emerald-700 to-indigo-700' : 'from-slate-900 to-indigo-900'} text-white flex justify-between items-center shadow-md flex-shrink-0">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-xl bg-white/20 backdrop-blur flex items-center justify-center text-xl shadow-inner">
                💬
              </div>
              <div>
                <div class="flex items-center gap-2">
                  <h3 class="text-base font-extrabold tracking-tight">WhatsApp Recovery Templates Library</h3>
                  <span class="px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider ${isPro ? 'bg-amber-400 text-slate-950' : 'bg-white/20 text-white'}">
                    ${isPro ? '👑 13 Pro Templates Unlocked' : 'Starter Mode (1 Free / 12 Pro Locked)'}
                  </span>
                </div>
                <p class="text-xs text-white/80 mt-0.5">High-converting collection scripts written specifically for Indian SMB payment follow-up</p>
              </div>
            </div>
            <button onclick="Modals.close()" class="text-white/80 hover:text-white text-2xl font-bold p-1">&times;</button>
          </div>

          <!-- Plan Notice / Banner -->
          <div class="p-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 flex-shrink-0">
            <div class="text-xs text-slate-600 flex items-center gap-2">
              <span class="font-bold text-slate-800">Previewing for Debtor:</span>
              <select id="wa-lib-cust-select" onchange="Modals.changeLibraryCustomer(this.value)" class="text-xs font-bold border border-slate-300 rounded-lg px-2.5 py-1.5 bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 max-w-xs">
                ${customers.map(c => `
                  <option value="${c.CustomerID}" ${c.CustomerID === selectedCust.CustomerID ? "selected" : ""}>
                    ${Utils.escapeHtml(c.CustomerName)} (₹${Number((c.metrics && c.metrics.totalOutstanding) || 0).toLocaleString("en-IN")})
                  </option>
                `).join("")}
              </select>
            </div>
            ...
            ${!isPro ? `
              <button onclick="Modals.openUpgradeModal('Unlock 13+ High-Converting WhatsApp Templates Library')" class="px-3.5 py-1.5 rounded-lg text-xs font-extrabold text-white bg-indigo-600 hover:bg-indigo-700 active:scale-95 shadow-sm transition-all flex items-center gap-1.5 self-start sm:self-auto">
                <span>⚡ Upgrade to Sarthi Pro for All Templates</span>
              </button>
            ` : `
              <span class="text-xs font-bold text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200">
                ✓ Sarthi Pro Plan Active
              </span>
            `}
          </div>

          <!-- Template Cards Container (Scrollable) -->
          <div class="p-5 overflow-y-auto space-y-4 flex-1">
            ${Object.keys(templates).map((key, idx) => {
              const t = templates[key];
              // On Starter plan, only template 1 (gentle) is unlocked, others require Growth Plan
              const isLocked = !isPro && key !== "gentle";

              return `
                <div class="p-4 rounded-xl border ${isLocked ? 'border-dashed border-slate-300 bg-slate-50/70' : 'border-slate-200 bg-white hover:border-emerald-300 shadow-sm'} transition-all relative">
                  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2.5">
                    <div class="flex items-center gap-2">
                      <span class="text-xs font-extrabold text-slate-900">${t.title}</span>
                      <span class="px-2 py-0.5 rounded text-[10px] font-bold ${isLocked ? 'bg-slate-200 text-slate-600' : 'bg-emerald-50 text-emerald-700 border border-emerald-200'}">
                        ${t.badge}
                      </span>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                      ${isLocked ? `
                        <button onclick="Modals.openUpgradeModal('Unlock ${t.title}')" class="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-amber-100 hover:bg-amber-200 text-amber-900 border border-amber-300 flex items-center gap-1 transition-all">
                          <span>🔒 Pro Locked (Upgrade)</span>
                        </button>
                      ` : `
                        <button onclick="Modals.copyArbitraryText('${encodeURIComponent(t.text)}')" class="px-2.5 py-1 rounded-lg text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 transition-colors flex items-center gap-1">
                          <svg class="w-3 h-3 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3"/></svg>
                          <span>Copy</span>
                        </button>
                        <button onclick="Modals.sendTemplateDirect('${phone}', '${encodeURIComponent(t.text)}', '${selectedCust.CustomerID}')" class="px-3 py-1 rounded-lg text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm flex items-center gap-1.5 transition-all">
                  ...
                  <div class="p-3 rounded-lg ${isLocked ? 'bg-slate-100/80 text-slate-500 filter blur-[0.3px]' : 'bg-slate-50 text-slate-800'} text-xs leading-relaxed font-sans select-all border border-slate-200/60">
          ...
          <!-- Footer -->
          <div class="px-6 py-3 bg-slate-50 border-t border-slate-200 flex justify-between items-center text-xs text-slate-500 flex-shrink-0">
            <span>Templates automatically personalize debtor name, balance, and your company name.</span>
            <button onclick="Modals.close()" class="px-4 py-2 text-xs font-bold text-slate-700 bg-white hover:bg-slate-100 border border-slate-300 rounded-lg">
              Close
            </button>
          </div>
        </div>
  ...
  changeLibraryCustomer: function(customerId) {
    this.openWhatsAppTemplatesLibrary(customerId);
  },

  copyArbitraryText: function(encodedText) {
    const text = decodeURIComponent(encodedText);
    navigator.clipboard.writeText(text).then(() => {
      Toast.success("Template text copied to clipboard!");
    }).catch(() => {
      Toast.info("Please select and copy text manually");
    });
  },

  sendTemplateDirect: async function(phone, encodedText, customerId) {
    const text = decodeURIComponent(encodedText);
    const cleanPhone = String(phone || "").replace(/\D/g, "").slice(-10);
    if (!cleanPhone) {
      Toast.error("This debtor has no registered phone number");
      return;
    }
    ...
    if (customerId) {
      try {
        await Api.call("createFollowUp", {
          ...
          FollowUpType: "WHATSAPP",
          Outcome: "SENT_REMINDER",
          Remarks: "WhatsApp reminder sent: " + (text.length > 80 ? text.substring(0, 80) + "..." : text)
        });
        Toast.success("Follow-up recorded in history");
      } catch (e) {}
    }

    const url = Utils.getWhatsAppUrl(cleanPhone, text);
